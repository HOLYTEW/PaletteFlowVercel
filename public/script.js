...
function clearPaletteBars() {
    ...
    colorBarIds.forEach((id, index) => {
        ...
        // Remove all children (e.g., split-half elements)
        colorBar.innerHTML = '';
        ...
        // Remove split-related class if exists
        colorBar.classList.remove('split-color');

        // Add back a clean input
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    startButton.addEventListener('click', function () {
        if (selectedFilter) {
            ...
            setTimeout(() => {
                // Hide loading animation and all sections
                loading.classList.add('hidden');
                document.querySelectorAll('.canvas-container, .color-picker-container, .artCanvas-container, .web-mockup-container, .web-mockup-container-4, .thirdCanvas-container, .thirdInteriorCanvas-container, .fifthInteriorCanvas-container, .fifthApplicationCanvas-container, .thirdApplicationCanvas-container, .fourthCanvas-container, .fifthCanvas-container, .fourthApplicationCanvas-container, .fourthInteriorCanvas-container').forEach(section => {
